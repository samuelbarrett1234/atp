...
BOOST_AUTO_TEST_CASE(Test_Wound_On_XLock_Request)
{
	...
	BOOST_TEST(status == WorkerStatus::BLOCKED);  // waiting on worker 1 to be removed
...
BOOST_AUTO_TEST_CASE(Test_SLock_Blocks_XLock_With_Wound)
{
	//Wound-wait logic still applies here
...
BOOST_AUTO_TEST_CASE(Test_SLock_Blocks_XLock_With_Wait)
{
	...
	pLkMgr->get_status_freeze_if_ready(0, &status);
	BOOST_TEST(status == WorkerStatus::BLOCKED);  // waiting for worker 1 to be removed
